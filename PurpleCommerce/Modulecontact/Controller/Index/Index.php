 ...
 */
namespace PurpleCommerce\Modulecontact\Controller\Index;
...
class Index extends Action
{
    ...
    public function execute()
    {
        ...
        if($post['fname']){         
            $txt.='<tr><td><strong>Client Name</strong>:'.$post['fname'].''.$post['lname'].'</td></tr>';          
        }
        if($post['address']){           
        ...
        if($post['qnty']){
            $txt.='<tr><td><strong>Quantity</strong>:'.$post['qnty'].'</td></tr>';
        }
        if($post['occ']){
            $txt.='<tr><td><strong>Occasion</strong>:'.$post['occ'].'</td></tr>';
        }
        $txt.='</table>';
        //echo $txt;
        ...
        $to = 'nitin.sharma@purplecommerce.com';     
        ...
        $transport = $this->_transportBuilder->setTemplateIdentifier(9, $storeScope)
